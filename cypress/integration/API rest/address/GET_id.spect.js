...
describe('Testing for address resource', () => {
    ...
    it('GET - verify the response of the query to the address resource', () => {
        cy.request({
                method: 'GET',
                url: 'users/' + testNonAdminUser.id + '/addresses/' + id,
                headers: {
            ...
            .then((response) => {
                ...
                expect(response.body.objects.address).to.have.property('id')
                    //check the array of addresses within the response, it must have address property
                expect(response.body.objects.address).to.have.property('address')
                    //check the array of addresses within the response, it must have reference property
                expect(response.body.objects.address).to.have.property('reference')
                    //check the array of addresses within the response, it must have user_id property
                expect(response.body.objects.address).to.have.property('user_id')
            })
    });
    ...
    it('query test - take all records from address', () => {
        cy.task('queryDb', 'SELECT * FROM addresses WHERE user_id =' + testNonAdminUser.id + ' AND id=' + id)
    ...
    it('GET - compare the id address, between db and api', () => {
        cy.request({
            ...
            .its('body.objects.address.id').should('be.equal', row['id']);
    })
    ...
    it('GET - compare the address address, between db and api', () => {
        cy.request({
            ...
            .its('body.objects.address.address').should('be.equal', row['address']);
    })
    ...
    it('GET - compare the reference address, between db and api', () => {
        cy.request({
            ...
            .its('body.objects.address.reference').should('be.equal', row['reference']);
    })
    ...
    it('GET - compare the user_id address, between db and api', () => {
        cy.request({
            ...
            .its('body.objects.address.user_id').should('be.equal', row['user_id']);
    })
})