...
describe('Testing for order resource', () => {
    ...
    it('query test - take all records from address', () => {
        cy.task('queryDb', 'SELECT * FROM details WHERE order_id =' + orderID).then(function(result) {
            allRow = result;
        })
    })
    ...
    it('GET - compare quantity of records from addresses, between db and api', () => {
        cy.request({
            ...
            .its('body.objects.details').should('have.length', allRow['length']);
    })
    ...
    it('GET - verify the response of the query to the order resource', () => {
        cy.request({
            ...
            .then((response) => {
                ...
                expect(response.body.objects.details).to.exist
                    //check the array of order within the response, it must have id property
                expect(response.body.objects.details[0]).to.have.property('id')
                    //check the array of order within the response, it must have ordered_quantity property
                expect(response.body.objects.details[0]).to.have.property('ordered_quantity')
                    //check the array of order within the response, it must have unit_price property
                expect(response.body.objects.details[0]).to.have.property('unit_price')
                    //check the array of order within the response, it must have total_by_product property
                expect(response.body.objects.details[0]).to.have.property('total_by_product')
                    //check the array of order within the response, it must have food_id property
                expect(response.body.objects.details[0]).to.have.property('food_id')
                    //check the array of order within the response, it must have order_id property
                expect(response.body.objects.details[0]).to.have.property('order_id')

                // all parameters must match
                expect(response.body.objects.details[0]).to.have.property('id', allRow[0].id)
                expect(response.body.objects.details[0]).to.have.property('ordered_quantity', allRow[0].ordered_quantity)
                expect(response.body.objects.details[0]).to.have.property('unit_price', allRow[0].unit_price)
                expect(response.body.objects.details[0]).to.have.property('total_by_product', allRow[0].total_by_product)
                expect(response.body.objects.details[0]).to.have.property('food_id', allRow[0].food_id)
                expect(response.body.objects.details[0]).to.have.property('order_id', allRow[0].order_id)
            })
    });
